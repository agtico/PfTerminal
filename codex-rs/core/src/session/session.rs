...
impl SessionConfiguration {
    ...
    pub(crate) fn apply(&self, updates: &SessionSettingsUpdate) -> ConstraintResult<Self> {
        self.apply_with_auth_manager(updates, self.provider.auth_manager())
    }

    pub(crate) fn apply_with_auth_manager(
        &self,
        ...
        if let Some(model_provider_id) = updates.model_provider.clone() {
            ...
            next_configuration.provider = create_model_provider(provider, auth_manager.clone());
        ...
        if let Some(corrected_provider_id) = codex_model_provider_info::corrected_catalog_provider(
            ...
            config.model_provider = provider.clone();
            next_configuration.provider = create_model_provider(provider, auth_manager);
            next_configuration.original_config_do_not_use = Arc::new(config);
        }
